...
export function DiscountDialog() {
  ...
  const form = useForm<Inputs>({
    ...
    values: {
      percent: draft?.discount?.percent ?? 0,
      value: draft?.discount?.value ?? 0,
      total: draft?.discount?.total ?? 0,
    },
  })
  ...
  useEffect(() => {
    const editParam = searchParams.get("edit_discount")
    if (editParam) {
      setIsOpen(true)
    }
  }, [searchParams])
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleCancel}>
      <DialogContent>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} id="discount-form">
          <FieldSet>
            <FieldGroup className="grid grid-cols-2 gap-4">
                  ...
                  <Field data-invalid={fieldState.invalid}>
                    ...
                    <AmountInput
                      ...
                      onChange={(value) => {
                        ...
                        form.setValue("total", (Number(value) / 100) * totalMax)
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor={field.name}>Cash</FieldLabel>
                    <AmountInput
                      {...field}
                      id={field.name}
