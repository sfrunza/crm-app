...
import {
  calculateRequest,
  calculateRoutes,
} from "@/domains/requests/request.api"
...
type RequestState = {
  ...
  setField: <K extends keyof Request>(
    key: K,
    value: Request[K],
...
export const useRequestStore = create<RequestState>((set, get) => ({
  ...
  setField: (key, value, isCalculatorEnabled = false) => {
    ...
    if (ADDRESS_FIELDS.includes(key)) {
      ...
      routeDebounceTimer = setTimeout(async () => {
        try {
          const result = await calculateRoutes(requestId, {
            ...newUnsaved,
            save: false,
          })
    ...
    if (isCalculatorEnabled) {
      ...
      debounceTimer = setTimeout(async () => {
        try {
          const result = await calculateRequest(requestId, {
            ...newUnsaved,
            save: false,
          })
          set({ calculatedRequest: result, isCalculating: false })
  ...
  setCalculatedRequest: (request) => set({ calculatedRequest: request }),
