...
type StatusFilterKey =
  | Exclude<Status, "pending_info" | "pending_date" | "hold">
  | "all"

type StatusFilterOptionsMap = Record<StatusFilterKey, StatusFilter>

export const statusFilterOptionsMap: StatusFilterOptionsMap = {
  all: "All Requests",
  pending: "Pending",
  unconfirmed: "Not Confirmed",
  confirmed: "Confirmed",
...
export const STATUS_FILTER_OPTIONS: StatusFilterOption[] = Object.entries(
  statusFilterOptionsMap
).map(([key, label]) => ({
  label: label,
  value: key as keyof StatusFilterOptionsMap,
}))

type DateFilterOptions = Record<
  "booked_this_month" | "upcoming_this_month" | "upcoming_all" | "booked_all",
  string
>
...
export const DATE_FILTER_OPTIONS: DateFilterOption[] = Object.entries(
...
export const STATUS_TEXT_COLOR: Record<Status | "all", string> = {
  ...
  unconfirmed: "text-indigo-600 dark:text-indigo-400",
  ...
  canceled: "text-neutral-600 dark:text-neutral-400",
...
export const STATUS_SOFT_BG_COLOR: Record<Status | "all", string> = {
  ...
  unconfirmed: "bg-indigo-500/12 dark:bg-indigo-400/12",
  ...
  canceled: "bg-neutral-900/12 dark:bg-neutral-50/12",
...
export const TIME_OPTIONS_WITH_MERIDIEM = generateTimeOptions({
  withMeridiem: true,
})
