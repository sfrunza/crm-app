...
import type { RequestRoom } from "./request-room.types"
...
export function useCreateRequestRoom(
  options?: Omit<
    UseMutationOptions<
      RequestRoom,
      Error,
      { requestId: number; data: Partial<RequestRoom> }
    >,
    "mutationFn"
  >
) {
...
export function useUpdateRequestRoom(
  options?: Omit<
    UseMutationOptions<
      RequestRoom,
      Error,
      { requestId: number; id: number; data: Partial<RequestRoom> }
    >,
    "mutationFn"
  >
) {
  const { onSuccess, ...rest } = options ?? {}
  return useMutation({
    mutationFn: ({ requestId, id, data }) =>
      updateRequestRoom(requestId, id, data),
    ...rest,
    // onSuccess: (data, variables, context) => {
...
export function useDeleteRequestRoom(
  options?: Omit<
    UseMutationOptions<void, Error, { requestId: number; id: number }>,
    "mutationFn"
  >
) {
  const { onSuccess, ...rest } = options ?? {}
