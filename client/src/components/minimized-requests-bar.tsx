...
export function MinimizedRequestsBar() {
  ...
  return (
    <div className="absolute right-0 bottom-0 left-0 z-20 border-t bg-background">
      <ScrollArea className="w-full">
        <div className="flex items-center gap-1.5 px-3 py-1.5">
          {minimized.map((req) => (
            <div
              key={req.id}
              className="group flex shrink-0 cursor-pointer items-center gap-1.5 rounded-md bg-muted py-1 pr-1 pl-2.5 text-xs font-medium shadow-sm transition-colors hover:bg-accent"
              onClick={() => open(req.id)}
            >
