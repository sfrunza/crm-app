...
export async function createEntranceType(
  entranceType: Partial<EntranceType>
): Promise<EntranceType> {
  const res = await api.post(ENDPOINT, { entrance_type: entranceType })
  return res.data
}

export async function updateEntranceType(
  id: number,
  data: Partial<EntranceType>
): Promise<EntranceType> {
