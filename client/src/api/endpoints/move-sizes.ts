...
export async function createMoveSize(
  moveSize: Partial<MoveSize> & { image?: File | null }
): Promise<MoveSize> {
...
export async function updateMoveSize(
  id: number,
  moveSize: Partial<MoveSize> & { image?: File | null; remove_image?: boolean }
): Promise<MoveSize> {
  const formData = new FormData()
  const moveSizeData: Record<string, any> = { ...moveSize }
